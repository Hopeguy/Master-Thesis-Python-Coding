...
def fitness_func_LCOS(solution):
    ...
    return fitness_LCOS[0] #negative as the function want to maximize but we want the lowest value for LCOS in Euro / kWh
...
Sensitivity_factor = 0.8 #procental icnrease of CAPEX for both Power and Capacity
...
for i in range(10):
    ...
    upper_bound = [8000/8000, np.max(Energy_hourly_use)/np.max(Energy_hourly_use)]  #upper bound, first is for Capacity, second is for Power
    ...
    Result_10_tries[8].append(cost_investment[0]) #cost investment
...
save_file_name_std = f"Results\Firefly_case_2_ESS_LCOS\ESS_power_LCOS_etc\Firefly_case_2_ESS_LCOS_{iteration}_gen.csv"

save_file_name_schedule = f"Results\Firefly_case_2_ESS_LCOS\Charge_discharge_capacity\Firefly_case_2_ESS_LCOS_{iteration}_gen_Sch_year_10.csv"

save_file_name_std_sensitivity = f"Results\Sensitivity_analysis_case_2\Firefly\Firefly_case_2_ESS_LCOS_{iteration}_gen_sensitivity_-20%.csv"

save_file_name_schedule_sensitivity = f"Results\Sensitivity_analysis_case_2\Firefly\Firefly_case_2_ESS_LCOS_{iteration}_gen_Sch_year_20_sensitivity_-10%.csv"
...
plt.title("Capacity when Optimizing for LCOS")
...
save_file_name_capacity = f'Results\\Pictures_etc\\Firefly-case-2-LCOS-convergence\\Firefly_Case_2_LCOS_convergence_capacity_iteration_{iteration}.jpeg'
...
plt.xlabel("Iteration")
plt.ylabel("Power [kW]")
plt.title("Power when Optimizing for LCOS")
...
save_file_name_power = f'Results\\Pictures_etc\\Firefly-case-2-LCOS-convergence\\Firefly_Case_2_LCOS_convergence_power_iteration_{iteration}.jpeg'
...
save_file_name_NPV = f"Results\\Pictures_etc\\Firefly-case-2-LCOS-convergence\\Firefly_Case_2_LCOS_convergence_fitness_iteration_{iteration}.jpeg"
