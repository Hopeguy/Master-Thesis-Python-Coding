...
def fitness_func_LCOS(solution, solutionidx):
    ...
    return -fitness_LCOS  # returns LCOS in Euro/kWh positivt as we want to minimze it and the firefly algorithm wants to do that
...
for i in range(10):
    ...
    Result_10_tries[3].append(-solution_fitness)# fittness function negative when looking for LCOS in GA
    ...
    Result_10_tries[9].append(cost_investment)  # cost investment
...
tf = pd.DataFrame(data_schedule, columns=['charge', 'discharge', 'capacity',
                             'Energy_use', 'Average_daily_cost', 'Energy_hourly_cost',
                              'Charge_or_discharge', 'Gas_heating_still_needed', 'Gas_heating_before_ELH_installment'])
...
save_file_name_std = f"Results\Pygad_case_3_ESS_LCOS\ESS_power_LCOS_etc\Pygad_case_3_ESS_LCOS_{num_generations}_gen.csv"

save_file_name_schedule = f"Results\Pygad_case_3_ESS_LCOS\Charge_discharge_capacity\Pygad_case_3_ESS_LCOS_{num_generations}_gen_Sch_year_10.csv"

df.to_csv(save_file_name_std, index=False, )
tf.to_csv(save_file_name_schedule, index=False, )
    

save_file_name_fittnes_plot = f"Results\\Pictures_etc\\Pygad-case-3-LCOS-convergence\\fitness_LCOS_over_generation_{num_generations}_gen.jpeg"
save_file_name_genes_plot = f"Results\\Pictures_etc\\Pygad-case-3-LCOS-convergence\\Best_genes_LCOS_{num_generations}_gen.jpeg"
save_file_name_solution_rate_plot = f"Results\\Pictures_etc\\Pygad-case-3-LCOS-convergence\\New_Solution_rate_LCOS_{num_generations}_gen.jpeg"

ga_instance.plot_fitness(title= "GA Case 3: LCOS", xlabel= "Generation",
                ylabel="LCOS [Euro/kWh]", plot_type="plot", save_dir=save_file_name_fittnes_plot)
ga_instance.plot_genes(graph_type='plot', title= "GA case 3: Best Genes LCOS", solutions="best", save_dir=save_file_name_genes_plot)
ga_instance.plot_new_solution_rate(title = "Ga Case 3: New solution rate vs Generation LCOS", ylabel= "New solutions explored", save_dir = save_file_name_solution_rate_plot)
solution, solution_fitness, solution_idx = ga_instance.best_solution()
